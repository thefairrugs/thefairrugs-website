...
export default function ProductDetail({ product, relatedProducts }: Props) {
  ...
  const pricing = useMemo(() => {
    ...
    const result = computePrice(
      ...
      product.priceAdjustment || 0
    );
    return { ...result, sqft, sizeLabel, sizeInfo };
  }, [selectedSizeId, customSize, customW, customH, rugTypeId, discount, sizeMaster, pricingData, product.priceAdjustment]);
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "72px", alignItems: "start" }}>
            ...
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginBottom: "28px" }}>
                ...
                {/* Add to Cart */}
                {cartMsg && (
                  <div style={{ padding: "10px 14px", background: "#d1fae5", borderRadius: "8px", fontSize: "13px", fontWeight: 600, color: "#065f46" }}>
                    ✅ {cartMsg}
                  </div>
                )}
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                  <button
                    onClick={() => {
                      if (pricing.sqft === 0) { setCartMsg("Please select a size first"); setTimeout(() => setCartMsg(""), 2500); return; }
                      addItem({
                        id: `${product.id}-${selectedSizeId || "custom"}`,
                        productId: product.id,
                        ...
                        sqft: pricing.sqft,
                        unitPrice: pricing.displayPrice * quantity,
                        quantity: 1,
                      });
                    ...
                    style={{
                      padding: "16px", background: "var(--foreground)", color: "#fff",
                      border: "none", borderRadius: "var(--radius-full)", fontWeight: 700, fontSize: "12px",
                ...
                <a href={`${WA_BASE}?text=${waMessage}`} target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none" }}>
                  <button style={{
                    ...
                    Order via WhatsApp{pricing.sqft > 0 && <span>· {pricing.priceLabel}</span>}
                  </button>
                </a>
                <button onClick={() => setInquiryOpen(true)} style={{
                  ...
                  Send Inquiry / Get a Quote
                </button>
                <Link href="/custom-rug" style={{ textDecoration: "none" }}>
                  <button style={{
                    width: "100%", padding: "14px", background: "transparent",
                    color: "var(--foreground-muted)", border: "1.5px solid var(--border)",
                    borderRadius: "var(--radius-full)", fontWeight: 600, fontSize: "12px",
                    letterSpacing: "0.08em", textTransform: "uppercase", cursor: "pointer",
                  }}>
